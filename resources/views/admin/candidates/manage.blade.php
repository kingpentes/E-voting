        ...
        <header class="bg-white shadow-sm z-10">
            <div class="px-4 sm:px-6 lg:px-8 py-4 lg:py-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                    <div>
                        <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Kelola Kandidat</h1>
                        <p class="text-gray-600 mt-1 text-sm lg:text-base">{{ $candidates->count() }} kandidat dari
                            pemilu Anda</p>
                    </div>
                    <a href="{{ route('admin.candidates.create') }}"
                        class="px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold rounded-xl text-sm sm:text-base w-fit">
                        <span>+ Tambah Kandidat</span>
                    </a>
                </div>

                <!-- Filter Pemilu -->
                <form method="GET" action="{{ route('admin.candidates.index') }}" id="candidateFilterForm"
                    ...
                    <select id="election_id" name="election_id"
                        class="px-4 py-2 border border-gray-300 rounded-lg w-full sm:w-80"
                        onchange="document.getElementById('candidateFilterForm').submit()">
                                        ...
                                        <div>
                                            ...
                                            <div class="flex items-center space-x-4 mt-2">
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    <span
                                                        class="text-sm font-semibold text-blue-600">{{ $candidate->missions->count() }}
                                                        Misi</span>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
                                    <div>
                                        @if ($candidate->photo)
                                            <img src="{{ asset('storage/' . $candidate->photo) }}"
                                                alt="{{ $candidate->name }}" class="w-full rounded-xl">
                                        @else
                                    ...
                                    <div class="col-span-2">
                                        <div class="mb-6">
                                            ...
                                            <p class="text-gray-700 bg-blue-50 p-4 rounded-lg">{{ $candidate->visi }}
                                            </p>
                                        </div>
                                        <div>
