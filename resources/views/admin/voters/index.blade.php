        ...
        <main class="flex-1 overflow-y-auto p-8">
                ...
                <div class="bg-yellow-50 border-l-4 border-yellow-400 p-6 rounded-lg mb-6">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-yellow-700 text-sm mt-1">Buat pemilu terlebih dahulu untuk melihat daftar voter yang mendaftar menggunakan access code Anda.</p>
                        </div>
                    </div>
                </div>
            @else
                ...
                <div class="bg-blue-50 border-l-4 border-blue-400 p-6 rounded-lg mb-6">
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-blue-700 text-sm mt-1">Hanya voter yang mendaftar menggunakan access code <span class="font-mono font-bold">{{ $election->access_code }}</span> yang ditampilkan di sini.</p>
                        </div>
                    </div>
                </div>
            ...
            @if($voters->isEmpty())
                <div class="bg-white rounded-2xl shadow-md p-12 text-center">
                    ...
                    <p class="text-gray-500 mb-6">Voter akan muncul setelah mereka mendaftar menggunakan access code pemilu Anda</p>
                </div>
            @else
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            <thead class="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Voter</th>
                                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Code Digunakan</th>
                                    <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Status Vote</th>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                @if($voter->face_photo)
                                                    <img class="h-10 w-10 rounded-full object-cover" src="{{ asset('storage/' . $voter->face_photo) }}" alt="{{ $voter->name }}">
                                                @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-xs text-gray-500">{{ $voter->organization ?? 'Tidak ada organisasi' }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            $joinedAt = \Carbon\Carbon::parse($voter->participatingElections->first()->pivot->joined_at);
                                        @endphp
                                        <div class="text-sm text-gray-900">{{ $joinedAt->format('d M Y') }}</div>
                                        <div class="text-xs text-gray-500">{{ $joinedAt->format('H:i') }} WIB</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
