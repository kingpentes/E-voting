<x-admin-layout title="Detail Voter">
    ...
    <div class="flex-1 flex flex-col overflow-hidden w-full lg:w-auto">
        <header class="bg-white shadow-sm z-10">
            <div class="px-4 sm:px-6 lg:px-8 py-4 lg:py-6 flex flex-col sm:flex-row sm:items-center gap-4">
                <a href="{{ route('admin.voters.index') }}" 
                   class="flex items-center space-x-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg transition-colors w-fit">
                ...
                <div>
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900">Detail Voter</h1>
                    <p class="text-gray-600 mt-1 text-sm lg:text-base">Informasi lengkap pemilih</p>
                </div>
            </div>
        </header>
                    ...
                    <div class="bg-gradient-to-r from-purple-600 to-pink-600 px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
                        <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 text-center sm:text-left">
                            <div class="flex-shrink-0">
                                @if($voter->face_photo)
                                    <img class="h-20 w-20 sm:h-24 sm:w-24 rounded-full object-cover border-4 border-white shadow-lg" 
                                         src="{{ asset('storage/' . $voter->face_photo) }}" 
                                         alt="{{ $voter->name }}">
                                @else
                                    <div class="h-20 w-20 sm:h-24 sm:w-24 rounded-full bg-white flex items-center justify-center text-purple-600 font-bold text-2xl sm:text-3xl shadow-lg">
                                        {{ strtoupper(substr($voter->name, 0, 1)) }}
                                    </div>
                                @endif
                            </div>
                            <div class="text-white">
                                <h2 class="text-xl sm:text-2xl lg:text-3xl font-bold">{{ $voter->name }}</h2>
                                <p class="text-purple-100 mt-1 text-sm sm:text-base break-all">{{ $voter->email }}</p>
                                <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2 sm:gap-4 mt-3">
                                    <span class="px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs sm:text-sm font-medium">
                                        🗳️ Voter
                                    </span>
                                    <span class="px-3 py-1 bg-white/20 backdrop-blur-sm rounded-full text-xs sm:text-sm font-medium">
