...
<body class="font-sans antialiased bg-gradient-to-br from-blue-50 via-white to-indigo-50">
                    ...
                    @auth
                        @php
                            ...
                            if (Auth::user()->hasVotedIn($election->id)) {
                                $canVote = false;
                                $votingMessage = 'Anda sudah memberikan suara pada pemilu ini';
                                $messageType = 'success';
                            }
                            // Check if election has started
                            elseif ($election->start_date && $now->lt($election->start_date)) {
                                $canVote = false;
                                $votingMessage =
                                    'Pemilu belum dimulai. Voting akan dibuka pada ' .
                                    $election->start_date->format('d M Y') .
                                    ' pukul ' .
                                    ($election->start_time ?? '00:00') .
                                    ' WIB.';
                                $messageType = 'warning';
                            }
                            // Check if election has ended
                            elseif ($election->end_date && $now->gt($election->end_date)) {
                                $canVote = false;
                                $votingMessage =
                                    'Pemilu sudah berakhir pada ' .
                                    $election->end_date->format('d M Y') .
                                    ' pukul ' .
                                    ($election->end_time ?? '23:59') .
                                    ' WIB.';
                                $messageType = 'error';
                            }
                            ...
                            elseif ($election->status !== 'active') {
                                ...
                                $statusLabel = $statusLabels[$election->status] ?? $election->status;
                                $votingMessage = 'Pemilu tidak dalam status aktif. Status saat ini: ' . $statusLabel;
                                $messageType = 'warning';
                            }
                        @endphp
                        ...
                        @if ($canVote)
                            <div class="mt-10 pt-8 border-t-2 border-gray-100">
                                <form action="{{ route('voter.vote', ['code' => $election->access_code]) }}" method="POST"
                            ...
                            <div class="mt-10 pt-8 border-t-2 border-gray-100">
                                @if ($messageType === 'success')
                                    <div class="bg-green-50 border-2 border-green-200 rounded-xl p-6 text-center">
                                        <svg class="w-12 h-12 text-green-500 mx-auto mb-3" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                        </svg>
                                        <p class="text-green-800 font-semibold text-lg">{{ $votingMessage }}</p>
                                    </div>
                                @elseif($messageType === 'warning')
                                    <div class="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-6 text-center">
                                        ...
                                        <p class="text-yellow-800 font-semibold text-lg">{{ $votingMessage }}</p>
                                    </div>
                                @elseif($messageType === 'error')
                                    <div class="bg-red-50 border-2 border-red-200 rounded-xl p-6 text-center">
                                        <svg class="w-12 h-12 text-red-500 mx-auto mb-3" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z">
                                            </path>
                                        </svg>
                                        <p class="text-red-800 font-semibold text-lg">{{ $votingMessage }}</p>
                                    </div>
                                @endif
                            </div>
                        @endif
    ...
    <script>
        function confirmVote(candidateNumber, candidateName) {
            return confirm(
                `Apakah Anda yakin memilih kandidat nomor ${candidateNumber} - ${candidateName}?\n\nPilihan Anda tidak dapat diubah setelah dikonfirmasi.`
                );
        }
    </script>
</body>
