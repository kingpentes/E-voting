            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div class="flex justify-between items-center">
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="text-sm text-white hover:text-blue-100 font-medium px-4 py-2 rounded-lg hover:bg-white/10 transition duration-200">Logout</button>
                    </form>
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full">
            ...
            @if ($user->verification_status === 'pending' && $user->id_card)
                <div class="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p class="text-yellow-800">⏳ Verifikasi Anda sedang ditinjau oleh admin. Mohon tunggu persetujuan.
                    </p>
                </div>
            @endif
            ...
            @if ($user->verification_status === 'rejected')
                <div class="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
                    <p class="text-red-800">❌ Verifikasi ditolak:
                        {{ $user->rejection_reason ?? 'Tidak ada alasan diberikan' }}</p>
                    <p class="text-sm text-red-600 mt-2">Silakan kirim ulang data verifikasi Anda dengan informasi yang
                        benar.</p>
                </div>
            @endif
                ...
                <form action="{{ route('voter.verification.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="border-2 border-dashed border-purple-300 rounded-xl p-6 text-center bg-purple-50">
                            ...
                            <label for="id_card" class="cursor-pointer">
                                <svg class="w-12 h-12 mx-auto mb-3 text-purple-500" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="mb-6">
                        ...
                        <div class="bg-gray-900 rounded-xl p-6">
                            <video id="cameraStream" class="w-full max-w-md mx-auto rounded-lg hidden" autoplay
                                playsinline></video>
                    ...
                    <div class="mt-8">
                        <button type="submit"
                            class="w-full text-white font-semibold py-4 px-6 rounded-xl transform transition duration-200 hover:scale-[1.02] shadow-lg hover:shadow-xl"
